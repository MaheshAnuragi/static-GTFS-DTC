...
def make_all_routes(username,agency):
    ...
    routes_df = pd.DataFrame([(route.route_id, route.agency_id, route.route_short_name, route.route_long_name) for route in routes_data],
    ...
    model_list = list()
    for route_id, route_name in route_id_name_dict.items():
    ...
    #--------------------------------------------------
    check_data = AllRoutes.query.filter_by(user_id=user.id).all()
...
def plot(route_id,user_id,username):
    ...
    if len(stop_details) == 0:
        ...
        # stops_df = pd.read_csv(f'data/{username}/new_stops.txt', dtype={"stop_id": str,"stop_name":str},converters={"stop_id": str.strip,"stop_name":str.strip})
        # db_response = db.session.query(AllRoutes).filter_by(route_id=route_id,user_id = user_id).all()[0]
        # stop_id_list = literal_eval(db_response.stop_details)
        # k = [str(i[0]) for i in stop_id_list]
        # route_stops_df = stops_df.set_index('stop_id').loc[k].reset_index(inplace=False)[
                                        # ['stop_id', 'stop_name', 'stop_lat', 'stop_lon']]
        
        stops_df = pd.DataFrame([(stop.stop_id, stop.stop_name, float(stop.stop_lat), float(stop.stop_lon)) for stop in stops_data],
                                columns=['stop_id', 'stop_name', 'stop_lat', 'stop_lon'])
        # route_stops_df = stops_df[stops_df['stop_id'].isin(stop_details[:, 0])]
        ...
        # stop_ids = stop_details[:, 0]
        for k in range(len(points)):
            folium.Marker(points[k], popup=response[k][1], tooltip=response[k][1]).add_to(m)

        # poly_line = folium.PolyLine(points, color=color[0], weight=5, opacity=1).add_to(m)
...
def recreate_trips(trip_id_list,trip_freq,trip_speed,route_id):
    ...
    for i in range(len(trip_id_list)):
            ...
            if existing_trip:
                ...
                trip = trips(route_id=route_id,freq=frequency,service_id=1,trip_id =tripID,speed=speed,user_id=current_user.id)
                db.session.add(trip)
                db.session.commit()
            ...
            stop_sequence=0
            existed_data_stopTimes = stopTimes.query.filter_by(user_id=current_user.id,trip_id=tripID,arrival_time=arrival_time,departure_time=departure_time,stop_id=stop_id,stop_sequence=stop_sequence).all()     
            print("------------existed_data_stopTimes----------")
            print(existed_data_stopTimes)
            if existed_data_stopTimes == []:
                existed_data_stopTimes=stopTimes(user_id=current_user.id,trip_id=tripID,arrival_time=arrival_time,departure_time=departure_time,stop_id=stop_id,stop_sequence=stop_sequence)
            ...
            for x in range(1, len(response)): # in response i have stop_id and stop_name at 0 and 1 indexes
                ...
                data_stopTimes=stopTimes.query.filter_by(user_id=current_user.id,trip_id=tripID,arrival_time=arrival_time,departure_time=departure_time,stop_id=response[x][0],stop_sequence=x).all()
                if data_stopTimes == []:
                # The record doesn't exist, so add it to the `stopTimes` table
                    data_stopTimes = stopTimes(user_id=current_user.id,trip_id=tripID,arrival_time=arrival_time,departure_time=departure_time,stop_id=response[x][0],stop_sequence=x)
...
def stops_change():
    if request.method == 'POST':
        stop_id = request.form.get('stop_id2')
        stop_lat = request.form.get('lat_name2')
        stop_lon = request.form.get('long_name2')
        stop_name = request.form.get('stop_name2')
...
def stops_add():
    if request.method == 'POST':
        ...
        new_stop = stops(user_id=current_user.id, stop_id=stop_id, stop_lat=stop_lat, stop_lon=stop_lon, stop_name=stop_name)
...
def view_route(route_id):
    ...
    if request.method == 'POST':
        ...
        for trip_time in Times:
            ...
            if existing_trip:
                ...
                # Create a new instance of allTrips and store the values
                trip = trips(route_id=route_id,freq=frequency,service_id=1,trip_id =tripID,speed=speed,user_id=user.id)
                db.session.add(trip)
                db.session.commit()
            ...
            stop_id = response[0][0]
            stop_sequence=0
            existed_data_stopTimes = stopTimes.query.filter_by(user_id=user.id,trip_id=tripID,arrival_time=arrival_time,departure_time=departure_time,stop_id=stop_id,stop_sequence=stop_sequence).all()     
            print("------------existed_data_stopTimes----------")
            print(existed_data_stopTimes)
            if existed_data_stopTimes == []:
                existed_data_stopTimes=stopTimes(user_id=user.id,trip_id=tripID,arrival_time=arrival_time,departure_time=departure_time,stop_id=stop_id,stop_sequence=stop_sequence)
                db.session.add(existed_data_stopTimes)
            ...
            for x in range(1, len(response)): # in response i have stop_id and stop_name at 0 and 1 indexes
                ...
                data_stopTimes=stopTimes.query.filter_by(user_id=user.id,trip_id=tripID,arrival_time=arrival_time,departure_time=departure_time,stop_id=response[x][0],stop_sequence=x).all()
                if data_stopTimes == []:
                # The record doesn't exist, so add it to the `stopTimes` table
                    data_stopTimes = stopTimes(user_id=user.id,trip_id=tripID,arrival_time=arrival_time,departure_time=departure_time,stop_id=response[x][0],stop_sequence=x)
                    db.session.add(data_stopTimes)
    ...
    return render_template('views/view.html', map=folium_map, response=response, trip_times=trip_times, route_name=route_name,route_id=route_id,current_user=current_user.id)
...
def show_time_table(route_id, trip_time, current_user):
    ...
    trip_records = stopTimes.query.filter_by(trip_id=tripID,user_id=current_user).all()
    print('------trip_records----')
    print(trip_records)
...
def edit_route(route_id):
    ...
    return render_template('views/edit_route.html', response=response, points=points, route_name=route_name,route_id  = route_id,stops_dict= stops_dict,points_len=points_len)
...
def delete(route_id,stop_id):
    ...
    return render_template('views/edit_route.html', response=response, points=points, route_name=route_name,route_id  = route_id,stops_dict = stops_dict,points_len=len(points))
...
def updateList(route_id):
    ...
    #    #call previous listorder from data
       ...
       #set previous stops order according to received ordered from ajax      
        ...
        all_routes.status = 0
        db.session.commit()
        return jsonify({'route_name':route_name,'route_id':route_id,'points':points,'response':response,'points_len':points_len})
...
def new_add(route_id):
    ...
    if request.method == 'POST':
        ...
        stops_data = [stops(stop_id=item['stop_id'], stop_name=item['stop_name'], stop_lat=item['stop_lat'], stop_lon=item['stop_lon'], user_id=current_user.id) for item in data]
...
def save_route(route_id):
    ...
    return render_template('views/view.html', map=folium_map, response=response, route_name=route_name,route_id=route_id)
...
#    agency_id,route_id,route_long_name,route_short_name,route_type
    ...
    if request.method == 'POST':
        ...
        route_ind = routes(agency_id=agency_id,route_id=route_id,route_long_name = rt_name, route_short_name = rt_name,route_type=status,user_id=user_id)
...
def create_gtfs_files(user_id, username,agency):
    ...
    routes_list = [{'agency_id': route.agency_id,'route_id':route.route_id,'route_long_name': route.route_long_name,'route_short_name': route.route_short_name, 'route_type':route.route_type} for route in routes_data]
    trips_list = [{'route_id':trip.route_id,'service_id':trip.service_id,'trip_id':trip.trip_id,'speed': trip.speed,'shape_id': trip.shape_id, 'freq':trip.freq} for trip in trips_data]
    stops_list = [{'stop_id':stop.stop_id,'stop_code':stop.stop_code,'stop_name':stop.stop_name,'stop_lat': stop.stop_lat,'stop_lon': stop.stop_lon, 'zone_id':stop.zone_id} for stop in stops_data]
    stop_times_list = [{'trip_id':stopTimes.trip_id,'arrival_time':stopTimes.arrival_time,'departure_time':stopTimes.departure_time,'stop_id': stopTimes.stop_id,'stop_sequence': stopTimes.stop_sequence} for stopTimes in stop_times_data]
...
#---------------------------------------------------------------------new export fn-------------
# global target_dir
# def make_routes_df_dict(stops_df, route_id_stop_id_dict):
    
#     """
#     makes {route_id: stops_df} dictionary
#     :param stops_df: gtfs stops df - will need to be stops_h5_file_df
#     :param route_id_stop_id_dict: routeId:[stop_ids...] dictionary
#     :return: routeId:stops_df dictionary
#     """
#     res_dict = dict()
#     reindexed_stops_df = stops_df.set_index('stop_id')
#     for route_id, stop_id_list in route_id_stop_id_dict.items():
#         print(route_id)
#         try:
#             # if type(stop_id_list[0])== str:
#             #     stop_id_list = np.array(stop_id_list).astype(str)

#             # else:
#             stop_id_list = np.array(stop_id_list).astype(str)
#             print(stop_id_list)
                
#             # print("stype od stop id  ",type(stop_id_list[0]))
#             route_stops_df = reindexed_stops_df.loc[stop_id_list].reset_index(inplace=False)
#             print("route_stops_df\n",route_stops_df)
#             res_dict[route_id] = route_stops_df
#         except KeyError as e:
#             print("---------error : ",e, route_id)
        
#     print()
#     return res_dict


# def make_routes(routes_list, route_ids_dict, agency_id, old_routes=None, make_csv=True):
#     global target_dir
#     res_dict = defaultdict(list)
#     print(" ----route_ids_dict   ",agency_id)
#     # if old_routes is not None:
#     #     start_route_id = np.max(old_routes.route_id.values) + 1
#     # else:
#     #     start_route_id = 0

#     # print(routes_list)
#     for idx, route in enumerate(routes_list):
#         route_id = route_ids_dict[route]
#         # route_short_name, route_long_name, route_type, route_id, agency_id
#         route_name = route
#         res_dict['route_short_name'].append('')
#         res_dict['route_long_name'].append(route_name)
#         res_dict['route_type'].append('3')
#         res_dict['route_id'].append(route_id)
#         res_dict['agency_id'].append(agency_id)
#     res_df = pd.DataFrame.from_dict(res_dict)
#     if make_csv:
#         res_df.to_csv(target_dir + 'routes.txt', index=False)
#     return res_df
    

# def make_trips(routes=None, start_time='06:00:00', end_time='23:00:00', service_frequency_minutes=10, make_csv=True,
#                verbose=False, routes_file=None):
#     def get_trip_time_suffix():
#         start = datetime.strptime(start_time, "%H:%M:%S")
#         end = datetime.strptime(end_time, "%H:%M:%S")
#         trip_time = start
#         trip_times = list()
#         while trip_time <= end:
#             trip_times.append(trip_time.strftime('%H_%M'))
#             trip_time += timedelta(minutes=service_frequency_minutes)
#         return trip_times

#     if routes_file is not None:
#         routes = pd.read_csv(routes_file)
#     res_dict = defaultdict(list)
#     trip_time_suffixes = get_trip_time_suffix()
#     for row in routes.itertuples():
#         if verbose:
#             print("currently on ", row.route_long_name)
#         for trip_time in trip_time_suffixes:
#             res_dict["route_id"].append(row.route_id)
#             res_dict["service_id"].append(1)
#             res_dict["trip_id"].append(f'{row.route_id}_{trip_time}')
#             res_dict["shape_id"].append("")
#     res_df = pd.DataFrame.from_dict(res_dict)
#     if make_csv:
#         res_df.to_csv(target_dir + 'trips.txt', index=False)
#     return res_df


# def make_stop_times(trips, route_stops_df_dict,routes_df, travel_speed_kmph=15, make_csv=True):
    
#     # print("--------------route_stops_df_dict---------------------")
#     # print(route_stop_ids_dict)
#     # route_stops_df_dict.to_csv('temp_route_stops_df_dict.txt', sep='\t', index=False)
#     # with open('temp_route_stops_df_dict.txt', 'w') as file:
#     #     file.write('key\tvalue\n')  # Write the header
#     #     for key, value in route_stops_df_dict.items():
#     #         file.write(f'{key}\t{value}\n')
#     # exit()
    
    
#     def calc_travel_minutes(c1, c2):
#         km_distance = distance(c1, c2).km
#         too_far_flag = False
#         if km_distance > 5:
#             too_far_flag = True
#         # travel time in minutes
#         minutes = (km_distance / travel_speed_kmph) * 60
#         return minutes, too_far_flag

#     def make_timetable_skeleton(route_df):
#         trip_time_list = [timedelta(minutes=0)]
#         stop_sequences = route_df[['stop_lat', 'stop_lon']]
#         for i in range(1, len(stop_sequences)):
#             prev_stop = stop_sequences.iloc[i - 1].values
#             curr_stop = stop_sequences.iloc[i].values
#             travel_time, too_far = calc_travel_minutes(prev_stop, curr_stop)
#             # if too_far:
#             #     print(prev_stop, curr_stop, pd.unique(route_df.routeName), stop_names[i - 1], '||', stop_names[i])
#             trip_time_list.append(timedelta(minutes=travel_time))
#         return trip_time_list

#     def gtfsify_stop_time(st_time):
#         splitted_time = st_time.split(':')
#         hour = splitted_time[0]
#         minutes = splitted_time[1]
#         seconds = splitted_time[2]
#         hour = f"{int(hour) + 24}"
#         new_time = f"{hour}:{minutes}:{seconds}"
#         return new_time
    
    
#     route_id_count = 1
#     res_dict = defaultdict(list)
#     all_routes = list(route_stops_df_dict.keys())
#     n  = 500
#     header = True
#     print("***",header)
    
#     if os.path.exists(target_dir + 'stop_times.txt'):
#         os.remove(target_dir + 'stop_times.txt')
#     else:
#         pass

#     for x in range(0,len(all_routes),n):
#         print("link",x,x+n)    
#         for i, route_name in enumerate(all_routes[x:x+n]):
#             print("===================for stop times==================")
#             print(i / len(all_routes), route_name)
        

#             route_id = routes_df[routes_df.route_long_name == route_name].route_id.values[0]
#             print("+++ route id count +++  ",route_id_count,"    ",route_id)
#             route_id_count += 1
#             route_df = route_stops_df_dict[route_name]  # route_df with sequence
#             trip_time_skeleton = make_timetable_skeleton(route_df)
#             route_trips = trips[trips.route_id == route_id]
#             for index, trip_row in route_trips.iterrows():
#                 trip_id = trip_row.trip_id
#                 # trip_start_time = trip_row.trip_id.lstrip(str(route_id))
#                 str1  = trip_row.trip_id[len(str(route_id)):]
#                 trip_start_time = str1

#                 trip_start_time_dt_obj = new_func(trip_start_time)
#                 stop_time = trip_start_time_dt_obj
#                 for idx, row in enumerate(route_df.itertuples(), 0):
#                     try:
#                         stop_time += trip_time_skeleton[idx]
#                         stop_time_str = stop_time.strftime("%H:%M:%S")
#                         # stop_id = stop_code_to_id[row[5]]
#                         stop_id = row.stop_id
#                         if stop_time.date() != trip_start_time_dt_obj.date():
#                             stop_time_str = gtfsify_stop_time(stop_time_str)
#                         res_dict['trip_id'].append(trip_id)
#                         res_dict['arrival_time'].append(stop_time_str)
#                         res_dict['departure_time'].append(stop_time_str)
#                         res_dict['stop_id'].append(stop_id)
#                         res_dict['stop_sequence'].append(idx)
#                     except KeyError as e:
#                         print(e, 'error')

#         res_df = pd.DataFrame.from_dict(res_dict)
#         if make_csv:
#             res_df.to_csv(target_dir + 'stop_times.txt',mode='a',index=False,header=header)
#             del res_df
#             del res_dict
#             gc.collect()
#             res_dict = defaultdict(list)
...
# def new_func(trip_start_time):
#     trip_start_time_dt_obj = datetime.strptime(trip_start_time, "_%H_%M")
#     return trip_start_time_dt_obj


# def make_gtfs(username,user_id):
#     global target_dir
#     stops_file = f'data/{username}/new_stops.txt'
#     route_sequences_db = "db.sqlite3"
#     db_conn = sqlite3.connect(route_sequences_db)

#     stops_df = pd.read_csv(stops_file, dtype={"stop_id": str,"stop_name":str},converters={"stop_id": str.strip,"stop_name":str.strip})
#     # stops_df = pd.read_csv(f"select * from stops_table where user_id={user_id}")
    
#     stops_df =stops_df.astype({"stop_id": str}) 

#     sequence_df = pd.read_sql_query(f"select route_name,agency_id, route_id,stop_details from all_routes where user_id = {user_id}", con=db_conn)
    
#     sequence_df['stop_details'] = sequence_df['stop_details'].str.replace("'", '"').apply(json.loads)
#     sequence_df['stop_details'] = sequence_df['stop_details'].apply(lambda x: [y[0] for y in x])
    
#     agency_id = sequence_df['agency_id'].values[0]

#     route_stop_ids_dict = pd.Series(data=sequence_df.stop_details.values,
#                                     index=sequence_df.route_name).to_dict()

#     route_ids_dict = pd.Series(data=sequence_df.route_id.values,
#                                     index=sequence_df.route_name).to_dict()


#     route_stops_df_dict = make_routes_df_dict(stops_df, route_stop_ids_dict)
#     target_dir = f'data/{username}/Exported_files/'
#     routes_list=list(route_stop_ids_dict.keys())
#     routes_df = make_routes(routes_list,route_ids_dict,agency_id)
#     trips_df = make_trips(routes_df)
#     stop_times_df = make_stop_times(trips_df, route_stops_df_dict,routes_df)
#     stops_df.to_csv(target_dir + 'stops.txt', index=False)
...
def uploader():
    if request.method == 'POST':
        ...
        # Delete the existing files for the agency and username
